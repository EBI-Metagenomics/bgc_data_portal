"""Asset projection unit tests — covers virtual-NRB construction and
...
def test_build_virtual_nrbs_chains_gecco_sanntis():
    """Overlapping GECCO + SanntiS predictions merge into one NRB; standalone
    antiSMASH becomes its own NRB. Both get negative ids."""
    virtual = build_virtual_nrbs(_seed_data())
    ...
    # Domains + CDS got attached to the correct virtual NRB.
...
def test_asset_domain_matrix_uses_fixed_vocab():
    ...
    # Chain NRB carries PF00001 + PF00002, antiSMASH NRB carries neither
...
def test_project_asset_against_synthetic_primary(monkeypatch, tmp_path):
    ...
    verify the four derived fields land on the virtual NRB."""
    ...
    data = _seed_data()
    virtual = build_virtual_nrbs(data)

    # Fake primary universe: two NRBs, one with the same domain composition
    ...
    pri_nrb_ids = np.array([1001, 1002], dtype=np.int64)
    ...
    from discovery.services.clustering import nrb_scoring as nrb_scoring_mod
    ...
    # No NonRedundantBGC / DashboardBgc rows in the test DB — patch the
    ...
    monkeypatch.setattr(discovery_models.NonRedundantBGC, "objects", _Empty())
    ...
    assert summary["n_nrbs"] == 2
    assert summary["projected"] is True

    roster = asset_cache.read_nrb_list("tok-test")
    ...
    detail = asset_cache.read_nrb_detail("tok-test", chain_row["id"])
    assert detail is not None
    assert detail["label"].startswith("NRB-A")
    assert len(detail["member_bgcs"]) == 2


def test_project_asset_no_clustering_run(monkeypatch):
    """Without a ClusteringRun, virtual NRBs still get persisted but
    ...
    roster = asset_cache.read_nrb_list("tok-no-run")
