...
def search_bgcs_by_keyword(keyword):
    ...
    if not keyword:
        return pd.DataFrame([])
    bgc_filter = BgcKeywordFilter(
        {"keyword": keyword}, queryset=Bgc.objects_with_contigs
    )
...
def search_bgcs_by_advanced(criteria):
    ...
    queryset = Bgc.objects.select_related(
        "bgc_detector", "bgc_class", "mgyc__assembly__biome"
    ).all()

    queryset = queryset.filter(
        Q(bgc_detector__bgc_detector_name__in=criteria.get("detectors"))
    )

    if criteria.get("bgc_class_name"):
        queryset = queryset.filter(
            bgc_class__bgc_class_name__icontains=criteria.get("bgc_class_name")
        )

    if criteria.get("mgyb"):
        queryset = MgybConverterFilter(field_name="mgyb").filter(
            queryset, criteria.get("mgyb")
        )

    if criteria.get("assembly_accession"):
        queryset = queryset.filter(
            mgyc__assembly__accession=criteria.get("assembly_accession")
        )
    ...
    if criteria.get("biome_lineage"):
        queryset = queryset.filter(
            mgyc__assembly__biome__lineage__icontains=criteria.get("biome_lineage")
        )

    if criteria.get("completeness"):
        queryset = queryset.filter(
            Q(partial__in=map(int, criteria.get("completeness")))
        )
    ...
    if criteria.get("protein_pfam"):
        ...
        pfam_queries = [
            Q(
                ...
                mgyc__metadata__start_position__lte=F(
                    "end_position"
                ),  # Metadata start is before or at BGC end
                mgyc__metadata__end_position__gte=F(
                    "start_position"
                ),  # Metadata end is after or at BGC start
            )
            for pfam in [
                _pfam.strip()
                for _pfam in re.split("[, ]", criteria.get("protein_pfam"))
            ]
        ]
...
def get_results_and_stats(pageless_query_params, sort_column=None, sort_order="asc"):
    ...
    current_advanced_form = (
        BgcAdvancedSearchForm()
        if query_params.get("keyword")
        else BgcAdvancedSearchForm(query_params or None)
    )
    # Try to get results from the cache
    results_df, result_stats = cache.get(
        pageless_query_params, (pd.DataFrame([]), None)
    )
    ...
    if not result_stats and query_params:
        ...
        result_stats = {
            ...
            "bgc_class_dist": (
                class_counter(results_df["bgc_class_names"])
                if not results_df.empty
                else {}
            ),
            "n_assemblies": (
                results_df["assembly_accession"].nunique()
                if not results_df.empty
                else 0
            ),
            "n_studies": (
                results_df["study_accession"].nunique() if not results_df.empty else 0
            ),
        }

        # Cache the results
        cache.set(
            pageless_query_params, (results_df, result_stats), timeout=CACHE_TIMEOUT
        )
